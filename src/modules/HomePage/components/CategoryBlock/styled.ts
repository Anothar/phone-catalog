...
const CategoryImageStyled = styled.div<ImgContainerType>`
  width: 100%;
  padding-top: 100%;
  background-image: url('${process.env.PUBLIC_URL}/img/category-phones.webp');
  ...
  ${({ variang }) => {
    switch (variang) {
      ...
      case 'second': {
        return css`
          background-image: url('${process.env.PUBLIC_URL}/img/category-tablets.png');
          background-size: 155%;
        `;
      }
      ...
      case 'three': {
        return css`
          background-image: url('${process.env.PUBLIC_URL}/img/category-accessories.webp');
          background-size: 130%;
        `;
      }
    }
